...
8A 已建立可信 GPU 基线和显存遥测；8B 已完成 À-Trous small-step shared tile 实验，但在 RTX 4060 Laptop GPU 上三个测试分辨率均明显慢于基线，因此默认仍使用 baseline。阶段 8 尚未完成。窗口标题显示阶段 8、最近有效 GPU Total、滚动 p95、输出尺寸、À-Trous 模式和 local VRAM usage/budget；PIX capture 可按 AS、Path Trace、Temporal、À-Trous 0/1/2/3 和 ToneMap 区分事件。
