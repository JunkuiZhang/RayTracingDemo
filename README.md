...
8A 已建立可信 GPU 基线和显存遥测；8B 的 À-Trous shared tile 实验因 RTX 4060 Laptop 三档实测回退而未采纳；8C 的 barrier/bind 优化已设为默认并保留 baseline 回退；8D 已实现两阶段 AS 初始化、TLAS 策略、profitable BLAS compaction 和 allocation 遥测，但仓库小模型没有触发真实 compact copy，AS 默认仍为 baseline；8E-1 已完成输出/内部尺寸解耦和按 fence 退休的资源代际切换；8E-2 已实现由已完成 GPU Total timestamp 驱动的动态分辨率、旧 generation 样本隔离、双重冷却和 measurement/lifetime 遥测，并完成首轮 review 修复。8G 已补齐 typed DebugView、fence-safe PNG、image_diff 和 bounded 显存 measurement；修复后短矩阵在 RTX 4060 Laptop 上生成的 20 个 30 秒 raw benchmark、54 张 capture/23 个 diff 和动态代际证据可用。后续 review 发现 runner 对三次运行取了最大值而非中位数；重算后 1920×1080 fixed p95 中位数为 7.67 ms，仍略高于 7.6632 ms，default dynamic 为 7.48 ms。五条 Debug 短验证还缺 raw 日志，1800/600 秒长测、真实 F1/F2/resize/最小化/恢复/hot-reload、公开 PBR/大模型和 PIX UI 证据也未执行，阶段 8 尚未完成。窗口标题显示阶段 8、命令记录模式、最近有效 GPU Total、滚动 p95、输出/内部尺寸、动态控制状态、generation、À-Trous 模式和 local VRAM usage/budget。
