...
阶段 6 首先建立 GPU 时空降噪所需的历史资源：`R16G16_FLOAT` 历史亮度矩、`R16G16_FLOAT` 相机运动矢量，以及扩展后的上一帧相机常量。RayGen 在渐进积累后执行 3×3 邻域颜色范围裁剪，并保存亮度一阶矩和二阶矩，为后续的历史有效性判断、方差估计和 À-Trous 空间滤波提供输入。相机移动或旋转时会清零样本计数，避免继续使用旧视角的统计结果。
