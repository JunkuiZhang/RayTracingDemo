...
H1–H4 最新状态（2026-08-09）：median/环境来源修复、五条 Debug raw 证据和 `2031abf` paired attribution 已完成。旧正式 fixed 1080p/default dynamic 的正确 p95 中位数为 7.67/7.48 ms；paired candidate/reference 为 7.42/7.34 ms、回退 1.089918%，属于 `HISTORICAL GATE PASS / NO PAIRED REGRESSION`。corrected `Recheck1080` 的 fixed/default dynamic median 为 7.07/7.93 ms，后者仍触发绝对门槛 FAIL。没有重跑 54/23 或 1800/600 秒长测；真实 F1/F2/resize/最小化/恢复/hot-reload、公开 PBR/大模型和 PIX UI 仍未执行，阶段 8 仍不得标记完成。

当前状态：**8A 可信 GPU 基线和显存遥测已实现；8B shared tile 实验已完成但因实测回退未采纳；8C 屏障批处理和重复绑定清理已采用 optimized 默认并保留回退路径；8D 两阶段 AS 初始化、TLAS 策略和 allocation 遥测已实现，但真实 compact copy、大模型及画面对比尚未验收，AS 默认仍为 baseline；8E-1 输出/内部尺寸解耦、固定 scale、generation fence 退休和 review 修复已完成；8E-2 GPU 时间控制器、旧代 timestamp 隔离、双重冷却和动态遥测已完成；8G 的 typed DebugView、capture、image diff 和 bounded 显存 measurement 已实现。修复后短矩阵的 20 个 30 秒 raw benchmark、54 张 128 SPP capture、23 个 diff 和动态 generation/history/显存证据可用；后续 review 发现 runner 对三次运行取了最大值而非中位数，重算后的 fixed 1080p/default dynamic p95 中位数为 7.67/7.48 ms，fixed 仍略高于 7.6632 ms。五条 Debug 短验证缺少可复核 raw 日志。下一包按 [`阶段8G短矩阵Review修复与1080p性能归因执行方案.md`](阶段8G短矩阵Review修复与1080p性能归因执行方案.md) 修统计、补证据并做同轮配对归因；1800/600 秒长测、真实 F1/F2/resize/最小化/恢复/hot-reload、公开 PBR/大模型和 PIX UI 仍未执行。8F 因 1080p Path Trace 占比约 43.7% 未达到 45% 入口，本轮不实施**。详细数据见 [`阶段8G总体验收记录.md`](阶段8G总体验收记录.md)，因此本阶段仍不能标记为完成。
...
8D 增加 `--acceleration-structure-mode baseline|optimized`：optimized 使用 Phase A build/postbuild readback、CPU committed-allocation 决策和 Phase B compact copy/TLAS build，并按真实动画能力决定 TLAS update 与 scratch 常驻。RTX 4060 Laptop 的 Cornell 和小型 glTF 均因 64 KiB allocation 粒度没有执行 compact copy，因此 AS 默认继续 baseline；本轮 glTF static/animated Release case 与 11 视图 AS exact diff 通过，但 Debug raw 留存、大 BLAS和人工画面对比仍待验收。8E-1 已增加 `output_extent`/`render_extent`、固定 `--render-scale`、ToneMap/F1 重采样、独立 generation heap、F2 档位、fence 退休和遥测。8E-2 在此基础上增加互斥 fixed/dynamic 配置、只消费已完成且 active-generation 匹配的 GPU Total timestamp、快降慢升控制器和 measurement/lifetime 遥测；本轮 1920×1080 default dynamic 三次 Total p95 为 7.48/7.64/7.43 ms（正确中位数 7.48 ms），fixed 三次为 7.63/7.69/7.67 ms（正确中位数 7.67 ms），3% 历史门槛仍为 FAIL。1800/600 秒长测和其他外部/人工债务仍未执行；阶段 8 仍未完成。

8G H1–H3 的可复核证据为：Debug summary `debug-20260809-193534-bc7a77b5`、corrected Recheck summary `20260809-195413-09b167f5`、paired summary `paired-1080-20260809-194700`。reference 固定为 `2031abf`，candidate/reference 正确 median p95 为 7.42/7.34 ms，回退 1.089918%；Recheck fixed/default dynamic 为 7.07/7.93 ms。H3 不改变任何渲染路径或默认模式，阶段 8 仍未完成。
