...
当前状态（2026-08-29）：11A–11F-R 的真实 RR、输入/生命周期修复、不透明边界稳定和 RTXPT-style 三稳定平面均已实现。nested 透射 continuation 与 Streamline shutdown 顺序已经修复；RTX 4060 Laptop 的干净构建通过 NRD/RR 1920×1080 Gate，最新 Total p95 分别为 `14.23/9.86 ms`，正式区间 `gpu_idle_wait_count == 0`。`--path-space-mode` 默认已切到 `auto`：SVGF 使用 legacy，NRD/RR 使用 stable planes，显式 legacy 回退仍保留；F3 在单个 generation transaction 中同时提交 denoiser 与 active path-space。扩展 Smoke 覆盖默认/显式矩阵并全部 PASS，详细证据见 [`阶段11RTXPT稳定平面准生产验收记录.md`](阶段11RTXPT稳定平面准生产验收记录.md)。Frame Generation 尚未生成中间帧；11G-A 已完成 SDK/部署/support ABI，11G-B 与 Codex review 修复已在 `8402494..6263fb3` 完成 linked interposer 单一 proxy 层、manual-hooking queue/swap-chain 路由、COM ownership、FG loaded-state ABI v6、默认 unload 边界和签名 OTA 兼容。下一步严格执行 [`阶段11FrameGeneration-11G-C-输入选项与生命周期执行方案.md`](阶段11FrameGeneration-11G-C-输入选项与生命周期执行方案.md)，第一次提交真实 FG 输入和 2x generated frame，但不提前实现 11G-D/E 的正式统计与验收。目标机 `slShutdown()` 仍可能卡在 NVIDIA NGX telemetry shutdown，该外部问题必须与渲染/FG 证据分开记录，不能通过跳过 shutdown 掩盖。阶段 11 整体仍不标记完成，也不能提前进入阶段 12。
