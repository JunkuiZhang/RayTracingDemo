...
当前状态（2026-09-11）：11A–11F-R 的真实 RR、输入/生命周期修复、不透明边界稳定和 RTXPT-style 三稳定平面均已实现。nested 透射 continuation 与 Streamline shutdown 顺序已经修复；RTX 4060 Laptop 的干净构建通过 NRD/RR 1920×1080 Gate，升级前最新 Total p95 分别为 `14.23/9.86 ms`，正式区间 `gpu_idle_wait_count == 0`。`--path-space-mode` 默认已切到 `auto`：SVGF 使用 legacy，NRD/RR 使用 stable planes，显式 legacy 回退仍保留；F3 在单个 generation transaction 中同时提交 denoiser 与 active path-space。扩展 Smoke 覆盖默认/显式矩阵并全部 PASS，详细证据见 [`阶段11RTXPT稳定平面准生产验收记录.md`](阶段11RTXPT稳定平面准生产验收记录.md)。11G-A/B/C 也已完成：linked interposer 单一 proxy、manual-hooking queue/swap-chain 路由、COM ownership、FG inputs/options 和显式生命周期状态机均已落地，2x FG 默认关闭。当前 SDK 已整体升级到 Streamline 2.14.1/NGX 310.9.1，RR 显式固定 DLSS 4.5 `Preset F`；feature 矩阵和 Release 构建通过，RTX 4060 Laptop 日志确认实际加载 2.14.1、选择 Preset F 并创建 RR context，详见 [`阶段11DLSS4.5与Streamline2.14.1升级记录.md`](阶段11DLSS4.5与Streamline2.14.1升级记录.md)。实时窗口现已增加显式 `--hdr`：FP16 scRGB 交换链、80 nit scRGB 标尺、可校准 paper white/峰值、显示器 HDR 校验、HDR-aware tone map、FG 格式传播和 SDR PNG 预览均已实现，见 [`阶段11HDR显示输出实施记录.md`](阶段11HDR显示输出实施记录.md)；默认 SDR 不变，跨显示器移动后的动态 HDR 重建尚未实现。升级后的剩余硬门禁是 RR 人工画质复验和聚焦 FG `status=0`、`numFramesActuallyPresented>=2` 证据；之后进入 11G-D 正式 base/display/generated/dropped/latency 统计，再做 11G-E 生命周期验收。目标机进程仍可能在 `slShutdown()` 的 NVIDIA NGX telemetry shutdown 内卡住；本次有界真机 smoke 也复现，因此“RR 已执行”和“进程正常退出”必须分别记录，不能互相冒充。阶段 11 整体仍不标记完成，也不能提前进入阶段 12。
