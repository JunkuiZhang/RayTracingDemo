...
H1–I3 最新状态（2026-08-09）：median/环境来源修复和五条 Debug raw 证据已完成，三个 runner 增加有界子进程等待。旧 schema v1 paired 的 raw 可用但 summary 没有绑定 commit/archive/EXE；修复后的 schema v2 run `paired-auth-1080-20260809-202926-33a32298` 从 commit archive 对 candidate `0fdcb7f`/reference `2031abf` 同法离线构建并验证 commit、tree、ZIP comment、archive/Cargo.lock/EXE SHA-256，p95 median 为 `7.91/7.84 ms`、差 `+0.892857%`，属于 `ABSOLUTE GATE FAIL / NO PAIRED REGRESSION`。同一 candidate 的 dynamic/fixed run `paired-resolution-1080-20260809-204146-98dcbc25` 为 `7.89/7.85 ms`、差 `+0.509554%`；dynamic 全程原生 1920×1080、零切换，属于 `ABSOLUTE GATE FAIL / NO DYNAMIC-MODE REGRESSION`。没有重跑 54/23、五条 Debug 或 1800/600 秒长测；阶段 8 仍不得标记完成。

当前状态：**8A 可信 GPU 基线和显存遥测已实现；8B shared tile 实验因实测回退未采纳；8C 屏障/绑定优化已采用 optimized 默认；8D 两阶段 AS 和 allocation 遥测已实现但大 BLAS compact-copy 尚未验收，AS 默认仍为 baseline；8E-1/8E-2 的资源代际与动态分辨率控制器已完成；8G 的 typed DebugView、capture/image diff、bounded 显存 measurement、Debug raw 和自认证短配对证据可用。20 个 30 秒历史 benchmark、54 张 128 SPP capture、23 个 diff 仍有效。最新同机交错配对证明没有超过 3% 的 commit 或 dynamic-mode regression，但绝对 `7.6632 ms` 历史门槛仍失败。1800/600 秒、真实 F1/F2/resize/最小化/恢复/hot-reload、公开 PBR/大模型和 PIX UI 仍未执行；8F 因入口占比不足不实施**。详细数据见 [`阶段8G总体验收记录.md`](阶段8G总体验收记录.md)，因此本阶段仍不能标记为完成。
...
8D 增加 `--acceleration-structure-mode baseline|optimized`：optimized 使用 Phase A build/postbuild readback、CPU committed-allocation 决策和 Phase B compact copy/TLAS build，并按真实动画能力决定 TLAS update 与 scratch 常驻。RTX 4060 Laptop 的 Cornell 和小型 glTF 均因 64 KiB allocation 粒度没有执行 compact copy，因此 AS 默认继续 baseline；本轮 glTF static/animated Release case、Debug raw 和 11 视图 AS exact diff 通过，但大 BLAS和人工画面对比仍待验收。8E-1 已增加 `output_extent`/`render_extent`、固定 `--render-scale`、ToneMap/F1 重采样、独立 generation heap、F2 档位、fence 退休和遥测。8E-2 在此基础上增加互斥 fixed/dynamic 配置、只消费已完成且 active-generation 匹配的 GPU Total timestamp、快降慢升控制器和 measurement/lifetime 遥测；历史 1920×1080 default dynamic 三次 Total p95 为 7.48/7.64/7.43 ms（正确中位数 7.48 ms），fixed 三次为 7.63/7.69/7.67 ms（正确中位数 7.67 ms）。最新同机模式配对差值仅 `+0.509554%`，没有 dynamic-mode regression，但绝对历史门槛仍为 FAIL。1800/600 秒长测和其他外部/人工债务仍未执行；阶段 8 仍未完成。

8G 的可复核证据为：Debug summary `debug-20260809-193534-bc7a77b5`、corrected Recheck `20260809-195413-09b167f5`、自认证 commit paired `paired-auth-1080-20260809-202926-33a32298` 和 dynamic/fixed paired `paired-resolution-1080-20260809-204146-98dcbc25`。旧 schema v1 paired `paired-1080-20260809-194700` 仅保留为 raw 历史；新证据链与执行细则见 [`阶段8G配对归因Review修复与动态模式归因执行方案.md`](阶段8G配对归因Review修复与动态模式归因执行方案.md)。本包未改变任何渲染路径或默认模式，阶段 8 仍未完成。
...
- RTX 4060 Laptop GPU 上以 1920×1080 输出、动态内部渲染达到 GPU Total p95 不高于 16.67 ms；默认内部尺寸不低于约 1280×720。
- 无每帧 CPU/GPU 强制同步。
- 显存占用低于预算的 70%。
- 固定分辨率画质不退化，Debug GPU Validation 和 30 分钟资源稳定性矩阵通过。
...
当前状态（2026-08-10）：9A–9F 已按独立 commit 实现；Codex review 又修正了 material factors/F0/视线方向、RGBA16F 2.5D motion 与静态子像素运动、REBLUR `NewFrame` 调用顺序、静态链接许可部署，以及 HLSL/Rust reconstruction UAV 15–17 错位。该错位曾把 diffuse hitT 当作 primary emissive 加回画面，是此前 NRD 全屏盐粒跳噪的根因；修复后 63→64 SPP 的 NRD 时域 MAE/RMSE 从 `32.1200/68.5116` 降至 `0.1003/2.2249`。后续镜面画质复验进一步加入 Owen-Sobol 时域采样、bounded GGX VNDF v3、NRD 主表面 Bayer probabilistic lobe + `AREA_3X3` hitT reconstruction，以及仅针对 specular-origin 后续面积光的 4-sample 预算；固定 64 帧镜中地面/玻璃顶面高频残差分别下降约 `5.5%/8.8%`。RTX 4060 Laptop 上 DLSS Quality + NRD 的 Total p50/p95 为 `5.57/6.35 ms`，Native + NRD 为 `9.67/10.23 ms`，均通过 16.67 ms 门槛。Release 真实窗口的 F3 往返、F1 十二视图回环、F2 四档回环、resize/最小化/恢复均通过；NRD prep/compose hot reload 缺口也已修复并在 Debug 下两次成功、InfoQueue 0。默认路径仍是 SVGF，NRD inactive pass 在 JSON 中保持 `null`。连续相机移动、动画拖影/闪烁和透明路径局部高光仍为 `PENDING USER VISUAL CONFIRMATION`；公开 PBR/大模型未提供，600/1800 秒长测未执行，因此阶段 9 仍不标记最终完成，但这些剩余项不阻塞阶段 10。详细 raw run-id、pass 数据和风险见 [`阶段9验收记录.md`](阶段9验收记录.md)。
...
当前状态（2026-09-12）：11A–11F-R 的真实 RR、输入/生命周期修复、不透明边界稳定和 RTXPT-style 三稳定平面均已实现。nested 透射 continuation 与 Streamline shutdown 顺序已经修复；RTX 4060 Laptop 的干净构建通过 NRD/RR 1920×1080 Gate，升级前最新 Total p95 分别为 `14.23/9.86 ms`，正式区间 `gpu_idle_wait_count == 0`。`--path-space-mode` 默认已切到 `auto`：SVGF 使用 legacy，NRD/RR 使用 stable planes，显式 legacy 回退仍保留；F3 在单个 generation transaction 中同时提交 denoiser 与 active path-space。扩展 Smoke 覆盖默认/显式矩阵并全部 PASS，详细证据见 [`阶段11RTXPT稳定平面准生产验收记录.md`](阶段11RTXPT稳定平面准生产验收记录.md)。11G-A/B/C/D 也已完成：linked interposer 单一 proxy、manual-hooking queue/swap-chain 路由、COM ownership、FG inputs/options、显式生命周期状态机和 application/display/generated/dropped 统计均已落地，2x FG 默认关闭。当前 SDK 已整体升级到 Streamline 2.14.1/NGX 310.9.1，RR 显式固定 DLSS 4.5 `Preset F`；feature 矩阵和 Release 构建通过，RTX 4060 Laptop 日志确认实际加载 2.14.1、选择 Preset F 并创建 RR context，详见 [`阶段11DLSS4.5与Streamline2.14.1升级记录.md`](阶段11DLSS4.5与Streamline2.14.1升级记录.md)。实时窗口现已增加显式 `--hdr`：为兼容 Streamline FG，交换链使用 RGB10、BT.2100/PQ 和 HDR10 metadata，tone map 提供可校准 paper white/峰值，并保留显示器 HDR 校验与 SDR PNG 预览，见 [`阶段11HDR显示输出实施记录.md`](阶段11HDR显示输出实施记录.md)；默认 SDR 不变，跨显示器移动后的动态 HDR 重建尚未实现。旧 FP16 scRGB 路径已由 RTX 4060 Laptop 上 HDR/SDR FG A/B 日志证明不满足 Streamline FG 契约并移除；修复后聚焦窗口已取得 `status=0`、`numFramesActuallyPresented=2`、warmup 4 的真实 HDR+FG 证据。11G-D 将每次成功 application Present 后的 SDK 实际显示数累积到标题和 benchmark JSON，并从 Reflex 原始报告给出 application-frame latency，缺失时不造数值；capture 也明确声明来自 application display output，详见 [`阶段11FrameGeneration-11G-D-统计与延迟实施记录.md`](阶段11FrameGeneration-11G-D-统计与延迟实施记录.md)。11G-E 自动部分也已完成：干净 HEAD 的 Release 五项矩阵全部 PASS，三个 FG-on case 均取得真实 2x、稳态 `gpu_idle_wait_count == 0`；Debug FG 正常退出且 InfoQueue 0。实现过程中修复了 Debug GUI 栈溢出、过期 UAV 布局断言和 FG `display_output` 在下一帧 Tone Map 前未从 `COPY_SOURCE` 回迁 UAV 的问题，完整证据见 [`阶段11FrameGeneration-11G-E-有界验收记录.md`](阶段11FrameGeneration-11G-E-有界验收记录.md)。目标机本次所有自动 case 均正常退出；历史上的 NGX telemetry shutdown hang 继续作为回归观察项。F5 两次往返、resize/最小化/恢复、运动画质、FrameView `MsBetweenDisplayChange` 和外部 Reflex 证据仍为人工 PENDING，因此阶段 11 整体仍不标记完成，也不能提前进入阶段 12。
